<template>
    <div class="PhotoBookmarked" ref="pic">
...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
        this.getPhoto(this.$store.state.user.userId,this.pageNum); 
        });  
...
.PhotoBookmarked::-webkit-scrollbar{
  width : 10px; 
  height: 1px;
}
.PhotoBookmarked::-webkit-scrollbar-thumb {
  border-radius: 10px;
  box-shadow   : inset 0 0 5px rgba(250,235,215, 0.5);
  background   : #c0bab2;
  }
  .PhotoBookmarked::-webkit-scrollbar-track {
  box-shadow   : inset 0 0 5px rgba(250,235,215, 0.5);
  border-radius: 10px;
  background   : #ededed;
  }
